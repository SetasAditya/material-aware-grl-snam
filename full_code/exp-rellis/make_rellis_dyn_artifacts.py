...
def _read_csv(path: Path) -> List[Dict[str, str]]:
    ...
    # Old runs used an inaccurate name for the fixed discrete semantic APF.
    # Canonicalize on read so historical artifacts remain usable.
    for row in rows:
        if row.get("method") == "neural_potential_field":
            row["method"] = "semantic_apf"
    return rows
...
def make_table2(args: argparse.Namespace) -> None:
    ...
    family = {
        ...
        "semantic_apf": "potential field",
        "dwa_semantic": "reactive",
        "mppi_semantic": "sampling",
...
def _select_spotlight_case(args: argparse.Namespace) -> str:
    rows = _read_csv(args.delayed_required_run / "dynamic_rollouts.csv")
    by_ep: Dict[str, Dict[str, Dict[str, str]]] = defaultdict(dict)
    for row in rows:
        if row["event_type"] == "delayed_required_escape":
    ...
    best_ep = None
    for eid, methods in by_ep.items():
        required = {"route_aware_stage2", "dwa_semantic", "blackbox_cvar"}
        if not required.issubset(methods):
            continue
        s2 = methods["route_aware_stage2"]
        dwa = methods["dwa_semantic"]
        blackbox = methods["blackbox_cvar"]
        s2_false = _f(s2, "route_deviation_delay") < 10.0
        dwa_false = _f(dwa, "route_deviation_delay") < 10.0
        bb_false = _f(blackbox, "route_deviation_delay") < 10.0
        if _f(s2, "success") < 0.5:
            continue
        score = (
            18.0 * _f(s2, "success")
            - 10.0 * _f(s2, "stuck")
            + 10.0 * float(not s2_false)
            + 8.0 * float(dwa_false)
            + 8.0 * float(bb_false)
            + 8.0 * _f(blackbox, "stuck")
            + 4.0 * (_f(dwa, "stale_exposure") - _f(s2, "stale_exposure"))
            + 4.0 * (_f(blackbox, "stale_exposure") - _f(s2, "stale_exposure"))
            + 3.0 * (_f(dwa, "path_length_ratio") - _f(s2, "path_length_ratio"))
            + 3.0 * (_f(blackbox, "path_length_ratio") - _f(s2, "path_length_ratio"))
        )
        if _f(blackbox, "success") < 0.5:
            score += 10.0
        if _f(dwa, "success") < _f(s2, "success"):
            score += 6.0
        if score > best_score:
            best_score = score
            best_ep = eid
    ...
    if best_ep is None:
        raise RuntimeError("No delayed-required spotlight case found")
    return best_ep
...
def _annotate_force_arrow(
    ...
    *,
    ...
    if len(path) < 2:
        return
    ti = max(1, min(len(path) - 1, t))
    p = path[ti]
    f = _movement_force(path, ti)
    ...
    ax.text(
        ...
        fontsize=7,
        weight="bold",
        bbox=dict(boxstyle="round,pad=0.16", fc="black", ec="none", alpha=0.62),
        zorder=9,
    )


def make_spotlight_figure(args: argparse.Namespace) -> None:
    eid = _select_spotlight_case(args)
    methods = ["stage1", "dwa_semantic", "blackbox_cvar", "route_aware_stage2"]
    ep, base_maps, paths, step_maps, spec = _roll_case(args, eid, "delayed_required_escape", methods)
    rollout = _rollout_lookup(_read_csv(args.delayed_required_run / "dynamic_rollouts.csv"))
    rows = {m: rollout.get(("delayed_required_escape", eid, m), {}) for m in methods}

    t_escape = spec.event_step + spec.open_delay
    t_pre = max(spec.event_step + 1, t_escape - 1)
    t_post = t_escape + 4
    risk_final = apply_dynamic_event(base_maps, spec, t_post, resolution=0.5)
    risk_pre = apply_dynamic_event(base_maps, spec, t_pre, resolution=0.5)

    method_meta = {
        "stage1": ("Geometry scaffold", "#9e9e9e", "ignores material update"),
        "dwa_semantic": ("DWA + semantic cost", "#377eb8", "false pre-activates"),
        "blackbox_cvar": ("Black-box CVaR", "#984ea3", "activates, then stalls"),
        "route_aware_stage2": ("Route-aware Stage 2", "#2ca25f", "suppresses, then escapes"),
    }

    fig = plt.figure(figsize=(15.6, 5.25), constrained_layout=True)
    gs = fig.add_gridspec(2, 4, height_ratios=[3.35, 0.42])
    for j, method in enumerate(methods):
        title, color, subtitle = method_meta[method]
        ax = fig.add_subplot(gs[0, j])
        ax.imshow(risk_final["risk_map"], cmap="magma", vmin=0, vmax=1)
        ax.contour(risk_final["hard_mask"], levels=[0.5], colors="white", linewidths=0.7)
        ax.contour(risk_pre["hard_mask"], levels=[0.5], colors="#8ecae6", linewidths=0.6, linestyles="--", alpha=0.85)

        path = paths[method]
        full_x, full_y = _path_xy(path)
        pre_x, pre_y = _path_xy(path, upto=t_pre + 1)
        ax.plot(full_x, full_y, color=color, lw=2.8, alpha=0.92, zorder=5)
        ax.plot(pre_x, pre_y, color="#f4d35e", lw=2.0, alpha=0.95, zorder=6, linestyle="--")
        ax.scatter([ep["start_rc"][1]], [ep["start_rc"][0]], marker="o", s=38, c="white", edgecolors="black", zorder=10)
        ax.scatter([ep["goal_rc"][1]], [ep["goal_rc"][0]], marker="*", s=95, c="#00f5d4", edgecolors="black", zorder=10)
        _annotate_force_arrow(ax, path, t_pre, "#f4d35e", "pre", scale=0.18)
        _annotate_force_arrow(ax, path, t_post, color, "post", scale=0.18)

        if method in rows and rows[method]:
            row = rows[method]
            metrics = (
                f"succ {float(row.get('success', 0.0)):.0f}, "
                f"stuck {float(row.get('stuck', 0.0)):.0f}, "
                f"false-pre {float(_f(row, 'route_deviation_delay') < 10.0):.0f}"
            )
        else:
            metrics = "scaffold replay"
    ...
    ax_bar.set_xlim(0, 1)
    ax_bar.set_ylim(0, 1)
    ax_bar.add_patch(plt.Rectangle((0.012, 0.66), 0.018, 0.18, color="#1a103d", transform=ax_bar.transAxes))
    ax_bar.add_patch(plt.Rectangle((0.036, 0.66), 0.018, 0.18, color="#fdae61", transform=ax_bar.transAxes))
    ax_bar.text(0.062, 0.75, "risk: dark low, bright high; white=hard after opening; cyan dashed=blocked before opening", fontsize=8.7, va="center")
    ax_bar.text(0.012, 0.43, "yellow dashed path/arrows = pre-escape behavior; colored solid path/arrows = post-escape behavior", fontsize=8.7, va="center")
    ax_bar.text(0.012, 0.13, "Selectivity signature: scaffold ignores the update, DWA/black-box move early, and route-aware Stage 2 waits before taking the newly feasible route.", fontsize=9.0, va="center", weight="bold")
    ax_bar.set_axis_off()
    fig.suptitle(f"RELLIS-Dyn delayed-required escape: one episode, four adaptation mechanisms (episode {eid})", fontsize=12.5)
    _save_figure(fig, args, "rellis_dyn_spotlight_four_methods")
