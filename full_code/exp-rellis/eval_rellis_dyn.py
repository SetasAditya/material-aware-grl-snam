...
# The old name overstated this baseline: it has no neural network, learned
# parameters, or training procedure.  Keep it as an input-only alias so old
# command lines remain reproducible while all newly written artifacts use the
# technically accurate canonical name.
METHOD_ALIASES = {"neural_potential_field": "semantic_apf"}
...
def _semantic_apf_step(
    ...
    *,
    ...
    """One-step descent of a fixed, discrete semantic potential.

    This is a semantic artificial-potential-field (APF) baseline, not a neural
    or learned controller.  At each 8-connected neighbor it evaluates fixed
    goal-attraction, semantic-risk, hard-clearance, and progress terms and
    chooses the lowest-energy non-hard cell.  ``goal`` is the next nominal
    scaffold waypoint supplied by the caller.  There is no learned parameter,
    sampled route-feasibility gate, latch, or replanning operation.
    """
    risk = maps["risk_map"]
    sdf = maps["sdf_hard"]
    hard = maps["hard_mask"].astype(bool)
...
# Backward-compatible private symbol for scripts that imported the old helper.
_potential_field_step = _semantic_apf_step
...
def _mppi_semantic_step(
    ...
    *,
    ...
    """Sampling-based local controller with semantic/risk running cost.

    This is a lightweight MPPI-style baseline: it samples finite-horizon local
    action sequences from the same BEV risk/SDF patch, scores them with a
    semantic cost, and executes the first action of the best trajectory. It has
    no graph search, no global route, and no Hamiltonian context gate.
    """
    risk = maps["risk_map"]
    sdf = maps["sdf_hard"]
    hard = maps["hard_mask"].astype(bool)
    ...
    dirs = DIRS_8.astype(np.float32)
    dir_units = np.asarray([_unit(d) for d in dirs], dtype=np.float32)
    align = np.maximum(0.05, np.exp(1.8 * (dir_units @ goal_vec)))
    probs = align / float(np.sum(align))
    seed = (
        int(cur[0]) * 73856093
        ^ int(cur[1]) * 19349663
        ^ int(goal[0]) * 83492791
        ^ int(goal[1]) * 2654435761
    ) & 0xFFFFFFFF
    ...
    for sample_idx in range(num_samples):
        pos = cur
        last_dir = prev_dir.copy()
        first: Optional[Tuple[int, int]] = None
        total = 0.0
        hit_hard = False
        for h in range(max(1, horizon)):
            if sample_idx < len(dirs) and h == 0:
                action_idx = sample_idx
            else:
                action_idx = int(rng.choice(len(dirs), p=probs))
            dr, dc = int(dirs[action_idx, 0]), int(dirs[action_idx, 1])
            nxt = (pos[0] + dr, pos[1] + dc)
            if not _in_bounds(nxt, shape):
                total += 250.0
                hit_hard = True
                break
            move = dir_units[action_idx]
            turn = 0.0 if float(np.linalg.norm(last_dir)) < 1e-8 else 1.0 - float(np.dot(last_dir, move))
            clearance = max(0.0, hard_margin_m - float(sdf[nxt]))
            progress = float(np.dot(move, _unit(np.asarray(goal, dtype=np.float32) - np.asarray(pos, dtype=np.float32))))
            total += (
                0.08 * _goal_dist(nxt, goal)
                + 7.0 * float(risk[nxt])
                + 22.0 * clearance
                + 160.0 * float(hard[nxt])
                + 0.7 * turn
                - 1.1 * progress
            )
            if hard[nxt]:
            ...
            if pos == goal:
                break
        total += 1.8 * _goal_dist(pos, goal)
        if hit_hard:
            total += 80.0
        if first is not None and total < best_score:
            best_score = total
            best_first = first
...
def _rollout(
    ...
    *,
    ...
    for t in range(max_steps):
        ...
        if method == "stage1":
        ...
        elif method == "semantic_apf":
            nominal, idx_stage1 = _path_follower_next(stage1_path, cur, idx_stage1)
            nxt = _semantic_apf_step(maps_t, cur, nominal, hard_margin_m=hard_margin_m)
            ...
            nxt = _mppi_semantic_step(
                maps_t,
                cur,
                goal,
                previous=prev,
                hard_margin_m=hard_margin_m,
                horizon=max(8, route_horizon),
            )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        ...
        help=(
            "Controllers to evaluate. The deprecated neural_potential_field "
            "input alias is accepted but results are written as semantic_apf."
        ),
    )
...
def main() -> None:
    args = parse_args()
    # Canonicalize before calculating work or writing metadata/results.  dict
    # preserves order and avoids duplicate rollouts if both names are supplied.
    args.methods = list(dict.fromkeys(_canonical_method(m) for m in args.methods))
