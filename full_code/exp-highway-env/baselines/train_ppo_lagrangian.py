...
def main() -> None:
    try:
        ...
        raise SystemExit(
            "train_ppo_lagrangian.py requires gymnasium, highway_env, and stable_baselines3."
        ) from exc

    from .safe_rl import SafeProgressCallback

    ap = argparse.ArgumentParser(description="Train a PPO-Lagrangian highway baseline.")
    ...
    ap.add_argument("--out", type=str, default="checkpoints/highway_ppo_lagrangian_baseline")
    ...
    print("[ppo_lagrangian] config", flush=True)
    ...
    while total_done < int(args.total_timesteps):
        ...
        print(
            f"[ppo_lagrangian] chunk {chunk_idx}: lambda={penalty_weight:.4f} "
        ...
        print(
            f"[ppo_lagrangian] eval: reward={stats.reward_mean:8.2f} "
    ...
    print("[ppo_lagrangian] saving final checkpoint...", flush=True)
    ...
    print(f"Saved PPO-Lagrangian model to {out_dir / 'model.zip'}", flush=True)
